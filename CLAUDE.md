...
- **時系列列は「今回列アンカリング＋全列マッピング」方式** (2026-07 改訂・Gemini/ChatGPT統合):
  真因は「今回列の座標系喪失＝セル所属誤認」(尿酸=今回値7.8を取りこぼし / 眼底=前々回の視神経陥凹を今回に混入)。
  対策: **①各セクションで「今回」列を1度だけ確定・固定 (Immutable Anchor) ②今回/前回/前々回を全列 物理位置通りに
  埋める (捨てずスペーサー化しグリッド維持) ③「迷ったら空」廃止→「迷ったら列を再確認」**。
  採用列(今回のみ納品)の取捨は後段プログラム: `toMeasurements` が **`今回値`列だけ pick し前回/前々回は破棄**。
  実装: `src/lib/scan-prompt.ts` (列スキーマ=検査項目/検査項目詳細/今回値/前回値/前々回値/単位/下限値/上限値/判定/備考) と
  `src/lib/elith-export.ts` の `toMeasurements` は**セットで整合を取る**。**検診に限らず全検査票で共通**。
