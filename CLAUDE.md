...
- **ホーム画面追加の案内は `src/components/AddToHomeCard.astro`** (2026-08)。開くたびに
  ...
  - **余白は 0 (ratio 1.0)**。PWABuilder Image Generator の出力 (2026-08 受領) を実測すると
    ロゴのインクが**横幅の 80.9%** で、旧設定 (ratio 0.76 = インク 61.5%) より 1.3 倍大きかった。
    小さいアイコンでの視認性はこの差が効くので同じ大きさに合わせた (再現後 81.1%)。
    maskable だけは直径 80% のセーフゾーンに収める必要があるため ratio 0.78
    (インク幅 0.631 / 対角 0.761 < 0.8)。
