...
- **ダッシュボード v1.0 (2026-08 発注者承認・モック → 実装)**。モック=Artifact
  ...
  並び順は **ウェルネス年齢＋AI疾病予防レポート → 検査結果 → 検査キットの進捗 → AIスキャン/AI問診 →
  医師との面談** の 1 カラム。旧 2 カラム (左=閲覧 / 右=状況) は廃止。
  - **名称**: 健康年齢 → **ウェルネス年齢** / AI 診断結果 → **AI疾病予防レポート** /
    オンライン相談 → **医師との面談（オプション）**。**内部識別子は変えない**
    (`health_age_scores` / `ui.health_age_followup` / `HealthAgeLatest` はそのまま)。
  ...
  - **未実装 (仕様未確定)**: モックにある「表示項目の設定 (20 項目から 8 つ / 18 項目から 8 つ)」。
    **どの項目を既定で出すかの選定基準が未決**のため入れていない。現状は
    `measurement-queries.ts` の `DEFAULT_TREND_ITEMS` (6 項目) を出す。決めたら
    `app_config` の新キー (例 `ui.trend_items`) に置く。
...
- **ボタンの色 = 同格の淡色ペア (2026-08 発注者指示)**。「データ」と「グラフ」は同格で、
  ...
  - **brand 塗りは 1 画面 1 つ = AI疾病予防レポートのタイル**だけ (Welltect の売りなので
