...
| `docs/lab/questionnaire_to_lab_csv_spec.md` | **AI問診回答→各社受渡ファイル(CSV/Excel) 変換仕様(実装用)**。共通設問No→各社必要行のマスターマッピング表+各社項目リスト+生成ルール(フリー/選択/範囲/複数)+PII確認事項。**血液(デメカル=リージャー)へ渡すファイルはExcel(.xlsx)** (Wellfort確認2026-08・従来CSVを訂正)。元=Wellfort問診項目マトリクスExcel |
