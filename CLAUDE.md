...
- **和文の組版 (2026-08 確定・長文が読めない指摘を受けて)**
  ...
  - **長文の行長は 38em (≒和文 38 字) で止める**。ワイド画面ではカード幅いっぱいに伸びて
    ...
    (表・画像は対象外)。
    - **【報告書の紙面では撤回 2026-09-03・発注者指示】正本 spec §4.13。**
      「全体に改行がオカシイ。**画面表示サイズに合わせて**」。**表・2 連大数字・数直線は
      紙面いっぱいを使う**のに本文だけ 38em で折り返し、右側が空いて折り返し位置が
      要素ごとにばらついて見えていた (実測 1440px: 紙面 1280 / 表 1079 / **本文 608**)。
      → `.report-prose` と `.report-sheet .md-region p/li` の上限を外し、
      **行長は器 (紙面の本文欄) が決める**。器自体が `app` 幅 (最大 1408px) で頭打ち。
      **`/coach` `/notices` の `.md-region` は 38em のまま** (対象外)。
      印刷も同経路でそろう (A4 30 → **29 ページ** / 使用率 94.9% → **97.8%**)。
      `verify:screen` の幅チェックを「608px 以下」から**「本文の折り返し == 本文欄の幅」**へ変更。
      **壊して落ちることを確認済み。**
