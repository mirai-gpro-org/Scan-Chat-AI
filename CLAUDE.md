...
- **Elith の AI 診断結果レポート (PDF) = パイプライン⑥・暫定実装 (2026-08-20)**。
  ...
  - **【仕様変更 2026-08-28・発注者指示】報告書は「Elith の PDF を見せる」から
    ...
    - **【前提】サービスの 2 本柱 = A 初期がんの早期発見 / B AI 診断による疾病予防アドバイス**
      (発注者確認 2026-08-28)。年1回の人間ドックでは初期がんを見逃しうるので年3回の検査を足し
      **年4回**で見る。**本報告書は B のみを担う** — 受領テキスト 20,080 字に「がん」「腫瘍」
      「マーカー」が **0 回**(腫瘍マーカーを5項目渡しているのに言及なし)。
      **A の材料は Elith でなくアプリ自身のスキャン結果**(腫瘍マーカー/ABC健診/画像所見/今回=空の行)。
      A の総合所見は **Elith へ出力を依頼中**(`status`付きフィールド・「評価した項目の範囲では」と
      評価項目の明示を必須の文型に)。**来なければカードごと非表示。記載が無いこと≠所見が無いこと。**
