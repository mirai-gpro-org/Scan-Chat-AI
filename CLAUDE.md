...
- **画面を覆うモーダルには進行状況を一律で出す (2026-08 発注者指示)**。全画面の選択画面は
  画面トップの進捗バーを完全に覆い隠し、「あとどのくらいで終わるか」が分からなくなっていた。
  - 実装は `choice-picker.ts` の `PickerProgress` / `progressHtml()`。
    **選択画面 (シート/全画面/検索) ・マトリクス・中止シートの 5 経路すべてに渡す**
    (`live-controller.ts` の `progressSnapshot()`)。
  - 出すのは **進捗バー + セクション名 + パーセント**。**表記は画面トップのヘッダ
    (`#progress-text`) と同じパーセントに統一する** (発注者指示 2026-08。当初「残り N 問」を
    出していて表記が割れていた)。値は `InterviewEngine.currentPercent()`。
    ※ 選択画面の下部に出る「↓ あと N 件」は**リストのスクロール量**であって進捗ではない (別物)。
  - **スクロールしないヘッダ側 (`.lp-head`) に置く**。リスト内に置くと流れて消える。
  - **進捗バーのトラックは `slate-200` から濃くしない**。情報を担うのは「塗り vs トラック」で
    brand-600 対 slate-200 = 3.50:1 (WCAG 1.4.11 の 3:1 を満たす)。濃くすると白地との差は
    上がるが塗りとの差が 2.73:1 / 1.78:1 まで落ちて**肝心の進捗が読めなくなる**。
    0% でバーが見えない件は、隣のパーセント表記 (brand-700 = 7.09:1) がテキストで同じ情報を
    伝えるため許容する。
  - **`openMatrixPicker` にも `root.dataset.layout = 'sheet'` を必ず付ける**。無いと
    `:not([data-layout='sheet'])` の出し分けに当たって**ハンドル・見出し・中止ボタン・
    スクリムが全部消える** (実測 2026-08)。
