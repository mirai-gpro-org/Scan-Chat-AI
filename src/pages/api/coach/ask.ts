...
以下のユーザーの AI 診断結果と検査データに基づいて、健康に関する質問に丁寧に答えます。
...
━━━ ② 一般的な健康・栄養・運動・睡眠の知識として答えられる質問 ━━━
   (個別の AI 診断結果に依存しないもの。例: 朝食の重要性、推奨睡眠時間、
    一般的な運動量等)
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!ctx) {
    return json({
      error: 'AI 診断結果が見つかりません。ダッシュボードで検査結果が表示されるかご確認ください。',
    }, 404);
  }
