...
- **孤立した過去列値の混入禁止 (今回の False-Value 対策・最重要)**: ある行で値が「前回/前々回」列
  にしか無く「今回」列が空欄なら、**その過去値を今回に採ってはならない。今回は空 ("-")**。
  直前の疎ページ対策は「**今回列に実在する値**」を守るための指示であって、**過去列にしか無い孤立値を
  今回へ繰り上げる根拠にはならない** (行に値が1つでも、それが過去列なら今回は空)。
  例: 眼圧「今回=空・前々回=16」→ 読み取った値="-" (16 は採らない)。
  例: 血清 RPR/TP抗体「今回=空・前々回=(-)」→ 読み取った値="-" ((-) は基準でも過去でも今回に採らない)。
...
- **孤立過去値の混入禁止 (今回の False-Value 対策)**: 値が過去列(前回/前々回)にしか無く今回列が空なら
  value="-"。疎ページ対策は「今回列の実在値」を守る指示であり、過去列の孤立値を今回に繰り上げる根拠に
  しない。例: 眼圧 今回空・前々回16 → value="-"。血清 RPR/TP抗体 今回空・前々回(-) → value="-"。
