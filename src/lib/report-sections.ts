 ...
 *
 * 【**見出しの文字列だけを持つ**】説明文 (リード) は持たない。
 *   設計ポリシーの本文「年 1 回の人間ドックの隙間を年 3 回の検査で埋め…」は
 *   **Wellfort に設計意図を説明するための文**であって、報告書の中身ではない。
 *   これを紙面に載せると **当社が書いた散文**になり、確定事項
 *   「報告書は Elith の出力以上の表記・表現を一切しない」(spec §1.0) に反する。
 *   ポリシーが求めているのは **2 本柱が構造として最初に見えること**であって、
 *   ポリシー文の掲載ではない。
 *
 * 【実装事故の記録 (2026-08-29)】
 *   ① 当初これを描かず、`axis` を配列を分けるための変数にしか使っていなかった。
 *      A に材料が無い検体 (タイプ 2) で **A 軸の痕跡がゼロ**になり、報告書が B から始まっていた。
 *   ② 次に、設計ポリシーの説明文をそのまま紙面へ貼った。**上記のとおり誤り。**
 *   **枠(軸)は見出しだけ。中身は Elith の出力だけ。**
 */
export const REPORT_AXES = [
  { key: 'A' as const, title: '初期がんの早期発見' },
  { key: 'B' as const, title: 'AI 診断による疾病予防アドバイス' },
] as const;
