...
export const CONFIG_SPECS: ConfigSpec[] = [
  ...
  // AI疾病予防報告書「初期がんの早期発見」のフォールバック文言 (spec §4.0.1)。
  // **既定は空 = 非表示** (§0.3「文言が未確定 → キーを空で用意。空なら非表示。決まったら admin から入れる」)。
  // 本命は Elith に書いてもらうこと (§10.1 E-1)。当社の定型文はフォールバックであり、
  // **味気なさは意図的に残す** — 妥協した文言で埋めると Elith への依頼の必要性が見えなくなるため。
  { key: 'ui.cancer_screening_not_included', type: 'string', group: '報告書', label: 'がん早期発見: 予備の文言', default: '',
    description: 'Elith が「初期がんの早期発見」に何も書かなかった回に出す当社の定型文。空なら文言を出さない'
      + ' (軸の見出しは設計ポリシーにより常に出る)。'
