...
export async function loadDashboard(diagnosticUserId?: string | null, viewerIsAdmin?: boolean): Promise<DashboardData | { error: string }> {
  ...
  try {
    ...
    if (!sb) {
      ...
      if (demoFallbackEnabled(uid, viewerIsAdmin)) return buildDemoDashboard(uid, null);
    ...
    if (appUserErr || artErr || resErr) {
      if (demoFallbackEnabled(uid, viewerIsAdmin)) return buildDemoDashboard(uid, appUser?.display_name_cache ?? null);
    ...
    if (demoFallbackEnabled(uid, viewerIsAdmin) && results.length === 0 && uid !== DEFAULT_USER) {
      const [
        { data: demoArtifacts },
        { data: demoResults },
      ] = await Promise.all([
    ...
    if (demoFallbackEnabled(uid, viewerIsAdmin) && results.length === 0) {
    ...
    if (demoFallbackEnabled(uid, viewerIsAdmin) && safeBundle.shipments.length === 0) {
...
export async function getMetricTrend(
  ...
  const sb = getServerSupabase();
  if (!sb) return demoFallbackEnabled(diagnosticUserId, viewerIsAdmin) ? demoMetricTrend() : [];
  ...
  if (out.length === 0 && demoFallbackEnabled(diagnosticUserId, viewerIsAdmin)) return demoMetricTrend();
  return out;
  } catch {
    return demoFallbackEnabled(diagnosticUserId, viewerIsAdmin) ? demoMetricTrend() : [];
  }
 ...
 */
export async function getHealthAge(diagnosticUserId: string, viewerIsAdmin?: boolean): Promise<HealthAgeSummary> {
  const sb = getServerSupabase();
  if (!sb) return demoHealthAge(diagnosticUserId, viewerIsAdmin);
  try {
    // 型未生成テーブルのため any 経由。
    ...
    const { data, error } = await (sb.schema('diagnosis') as any)
      ...
      .limit(24);
    if (error || !data || data.length === 0) return demoHealthAge(diagnosticUserId, viewerIsAdmin);
    ...
    return demoHealthAge(diagnosticUserId, viewerIsAdmin);
  }
}
 ...
 */
function demoHealthAge(diagnosticUserId?: string | null, viewerIsAdmin?: boolean): HealthAgeSummary {
  if (!demoFallbackEnabled(diagnosticUserId, viewerIsAdmin)) return { latest: null, trend: null };
  const points: MetricTrendPoint[] = [
