...
export function buildDemecalProductionInstaller(input: InstallerInput): InstallerResult {
  ...
  P('  try { if ($moved -and -not (Test-Path -LiteralPath $InstallRoot)) { Move-Item -LiteralPath $OldDir -Destination $InstallRoot -Force } } catch {}');
  P("  Fail 'SWAP_FAILED' $_.Exception.Message");
  P('}');
  P('try { if (Test-Path -LiteralPath $OldDir) { Remove-Item -LiteralPath $OldDir -Recurse -Force } } catch {}');
  P('');
