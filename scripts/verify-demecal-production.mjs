/**
 * `scripts/tests/demecal-production.tests.ps1` を走らせるだけのラッパ。
 *
 * 実行: npm run verify:demecal-production
 *
...
const TEST = resolve(ROOT, 'scripts/tests/demecal-production.tests.ps1');
...
if (!exe) {
  ...
  console.error('  PWSH=<pwsh のパス> npm run verify:demecal-production でも指定できます。');
  process.exit(1);
}
