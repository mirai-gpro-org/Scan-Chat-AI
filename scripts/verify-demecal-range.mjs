/**
 * `scripts/tests/demecal-range.tests.ps1` を走らせるだけのラッパ。
 *
 * 実行: npm run verify:demecal-range
 *
...
const TEST = resolve(ROOT, 'scripts/tests/demecal-range.tests.ps1');
...
if (!exe) {
  ...
  console.error('  PWSH=<pwsh のパス> npm run verify:demecal-range でも指定できます。');
  process.exit(1);
}
