...
| フォーム区分 | No | 主な取得元 | 備考 |
|---|---|---|---|
| 基本情報 | 0–8 | 識別番号=**整理番号**（採番=要確認§下記）／性別・生年月日=**customer(PII)**／受診日・身長・体重・BMI・腹囲=**健診スキャン** | 生年月日はPII方針と要調整（下記） |
...
**(D) 要確認（`laif_s3_secure_handoff_spec §0.2` と同一）**
- **整理番号（識別番号 No.0）の採番主体**：LAiF回答=「自社アプリで連番自動採番」 vs Wellfort=「仮名ID採番」→ どちらが採番し突合するか。
- **生年月日（No.3）のPII**：フォームは生年月日を要求するが Wellfort は PII 分離方針。**モデル必須か／年齢+性別で代替可か**を LAiF に確認（必須なら送信可否を Wellfort 判断）。
- **問診由来項目の充足**：既往歴/服薬(9–24)・生活習慣(25–61) は AI問診の設問網羅性に依存（不足設問は問診票側で補完）。
