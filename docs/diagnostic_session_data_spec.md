...
監査官モードで 10 列構成を出力する。最後の 2 列のうち `Audit/修正案` は Gemini の自己チェック専用で、後段で除去される。
...
| **Audit/修正案** | **Gemini が不整合を検出した場合の修正案。整合時は `-` (内部のみ)** |
| 単位 | 紙面通り |
| 下限値 | 紙面通り |
| 上限値 | 紙面通り |
| 判定 | 範囲外なら H/L、範囲内なら `-` |
| 備考 | 不整合検出時 `【不整合】/【欠落】/【混線】/【要確認】 理由`、無ければ `-` |

#### 確定 `scan_md` (Supabase / Elith 入力フォーマット)

クライアント側で **`Audit/修正案` 列のみを除去**した 9 列構成。これが `scan_artifacts.content` に入る。
...
- 「Audit/修正案」列は **本フォーマットに存在しない** (中間データ、永続化禁止)
