...
- **(e) 【ほぼ解消・要確認 1 点】顧客との突合キー**。動画で **`extchar03` = 画面の「ボックスナンバー」**
  （物理キットの箱番号）と判明した（§4.1.2）。Wellfort は発送側なので「どの箱を誰に送ったか」を
  自社で持てる＝**これが突合キー**。受け皿カラムも実在（`lab_tests.external_barcode`）。
  **残る確認は「Wellfort 側でボックスナンバーを実際に記録しているか」の 1 点だけ。**
  記録が無ければ、代わりに **PDF 2 ページ目の `認証キー`** で照合するか、発送記録の運用を足す。
