...
6. PCに `Q05-0010-000000result_{日付}_{件数}.csv` が保存される。
   - **⚠ デスクトップに保存しないこと（2026-08-31 追記）。** 専用PCは**デスクトップが
     OneDrive 配下**になっている（実測: `C:\Users\info\OneDrive\デスクトップ\…`）。
     ここへ置くと**個人情報を含む原本CSVが Microsoft のクラウドへ同期され**、
     下の 4. で削除しても**ごみ箱・バージョン履歴に残り得る**。
   - **ブラウザの既定の保存先（ダウンロードフォルダ）のままで良い。**
...
- **CSVをデスクトップに置かない**（専用PCはデスクトップが OneDrive 同期対象＝クラウドへ上がる）。
  詳細はステップ②-6 の注記。ダウンロードフォルダは同期対象外なので既定のままでよい。
</content>
