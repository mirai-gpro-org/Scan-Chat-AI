# デメカル 血液CSV RPA 運用設計 (専用PC・attended/unattended)

| 項目 | 内容 |
|---|---|
| 位置づけ | `docs/lab/demecal_auto_download_overview_spec.md`（方式選定）を受けた**実装/運用設計** |
| 前提(確定) | §6 自動アクセス**承認済**／専用PC(Pマーク)／デメカル証明書は**PCのOS/ブラウザ証明書ストア導入済**／担当者は**admin を Google 認証**でログイン |
| ⚠️ **有効な範囲** | **§1〜§3・§5・§6 のみ有効。§4（unattended）は失効**（正本 = [`demecal_unattended_spec.md`](./demecal_unattended_spec.md)） |
| 本命方式 | ~~案2: Power Automate Desktop~~ → **【方式確定 2026-08-31】PowerShell（PAD は不採用）**。専用PC 実測で 証明書つき接続 HTTP 200・ログイン画面は素の HTML フォーム（`demecal_powershell_probe_guide.md`） |
| 実装状況 | サーバ側(変換/S3/状態管理) と admin取り込みUI は**実装済**。残るは **PC側 PowerShell スクリプト(DL部)** と **`LAB_INTAKE_API_KEY`（未実装）** |

---

## 1. 役割分担（Pマーク × 鍵一元管理の両立）

鍵(AWS/Gemini)は **Vercel 本番 env のみ**。専用PCには**据え置きの鍵を置かない**(CLAUDE.md)。

```
[専用PC(証明書あり)]                 [wellfort /admin]          [Scan-Chat-AI API]        [S3]
 デメカルへ mTLS+ID/PW ログイン         (入口・認可)               (変換・状態・鍵)
   → 汎用CSVをDL ─────────────▶ CSV を POST ──────▶ elith-blood-csv ──▶ BloodTestData JSON
                                (Google認証 or            (SCAN_CHAT_AI_API_KEY)   → S3 putFiles
                                 x-intake-key)            demecal-state (last_to) ◀▶ S3 state
```

- **専用PC**: デメカルからの **DL のみ**（証明書はここにしか無い）。
- **Vercel(Scan-Chat-AI)**: 受領CSV→`BloodTestData` JSON化→S3、状態 `last_to` 管理（鍵はここだけ）。
- **認可**: attended=担当者の Google 短命トークン／unattended=取り込み専用キー `x-intake-key`。

---

## 2. API（実装済）

| エンドポイント(wellfort中継) | メソッド | 用途 | 認可 |
|---|---|---|---|
| `/api/admin/elith-blood-csv` | POST `{csvBase64, filename, idPrefix}` | CSV→JSON→S3。応答に `max_test_date` | Google認証 or `x-intake-key` |
| `/api/admin/demecal-state` | GET | `last_to` を読む | 同上 |
| `/api/admin/demecal-state` | POST `{last_to}` | `last_to` を**単調前進**で更新（過去日は据置、`force`で上書き） | 同上 |

- 状態は S3 `{prefix}state/demecal_last_to.json`。
- 上流(Scan-Chat-AI)側は従来どおり `Bearer ADMIN_API_KEY`。wellfort が鍵を保持し中継。

### env（追加）
- wellfort: `LAB_INTAKE_API_KEY`（無人RPA用の最小権限キー。未設定なら intake 認可は無効＝attendedのみ）。
- 既存: wellfort `SCAN_CHAT_AI_API_KEY` = Scan-Chat-AI `ADMIN_API_KEY`（同値）、`AWS_*`（Scan-Chat-AI）。

---

## 3. attended 運用（担当者クリック・鍵ゼロ）

1. 専用PCで **admin を Google 認証**でログイン → `/admin`（Elithバッチ画面）。
2. 「🩸 デメカルCSV 取り込み（attended）」カード:
   - `状態 last_to` を表示（次回DL開始日 = last_to+1 の目安）。
   - デメカルから手動DLした**汎用CSV**を選択 → **取り込み実行**。
3. 結果（件数・最新検査日）表示。成功で **`last_to` を最新検査日まで自動前進**。

> attended は担当者の Google トークンで認可＝**PCに常設の秘密を置かない**（Pマーク的に最もクリーン）。DL自体は当面手動。

---

## 4. unattended 運用（Power Automate Desktop + タスクスケジューラ）

> **【この節は上書きされた 2026-08-31】無人運用の正本は
> [`demecal_unattended_spec.md`](./demecal_unattended_spec.md)。**
> 発注者判断で**最初から無人で定期実行**する（段階導入は採らない）。
> 以下 §4.1〜§4.4 は経緯として残すが、**設計の正はあちら**。
>
> **【方式確定 2026-08-31】PAD は不要。PowerShell 方式で行く。**
> 専用PC 実測で 証明書つき接続 **HTTP 200** ／ ログイン画面は**素の HTML フォーム**
> （根拠と手順は `demecal_powershell_probe_guide.md`「実測結果」「ログインフォームの構造」）。
> 以下 §4.1 の「Chrome/Edge を起動…」はブラウザ操作前提の旧計画。
> **PowerShell では 3〜4 を「GET でトークン取得 → 同一セッションで POST → CSV 取得」に置き換える**
> （他のステップは変わらない）。§4.4 が現行の無人化設計。

### 4.1 フロー
```
毎週(例 月 早朝) タスクスケジューラ → Power Automate Desktop フロー起動
 1. GET /api/admin/demecal-state (x-intake-key)         → last_to 取得
 2. from = last_to + 1日 , to = 実行日 - N日            → 範囲決定
 3. Chrome/Edge を起動(OSストア証明書で自動選択) → dl.demecal.net にID/PWログイン
 4. 汎用CSV: 代理店Q05-0010 / 日付範囲 from〜to / 正常終了 / 見出しあり → 確認 → ダウンロード
 5. 保存したCSVを読み、POST /api/admin/elith-blood-csv (x-intake-key) → JSON化→S3
 6. 応答 max_test_date で POST /api/admin/demecal-state {last_to: max_test_date}（前進）
 7. 成否・件数をログ。失敗時はメール/Teams 通知（last_to は前進しない＝次回リトライで欠損防止）
```

### 4.2 無人化の要件・留意
- **PCの稼働**: OSストア証明書は導入ユーザーの「個人」ストア → **そのユーザーセッションで実行**が必要。完全無人（ログオフ状態）にするなら証明書を **LocalMachine ストア＋サービスアカウント**へ移し、Power Automate も**無人モード（要ライセンス）**。
- **取り込み専用キー**: `LAB_INTAKE_API_KEY` を **Windows 資格情報マネージャー**に保管（リポジトリ非保存・ローテーション可）。フル権限の admin キーは置かない。
- **日付基準**: 「日付範囲」が報告日基準か採取日基準かで取り漏れ対策が変わる（採取日基準なら `to = 実行日 - N日` のマージン＋直近数週間の差分再取得）。→ 先方確認（overview §9.1）。
- **キャッチアップ**: PC停止・障害でスキップしても、次回は `last_to` から継続＝取り漏れなし（状態が単調前進）。

### 4.3 段階導入
1. **attended を先行運用**（実装済・鍵ゼロで安全）。
2. Power Automate Desktop フローを構築 → **手動起動**で通し確認（範囲決定→DL→取り込み→last_to前進）。
3. タスクスケジューラで**週次自動化**＋監視。証明書失効/更新・画面変更の検知を運用手順化。

### 4.4 実測で出た 2 つの制約と、その潰し方（2026-08-31 確定）

> **§4.4 の内容は [`demecal_unattended_spec.md`](./demecal_unattended_spec.md) §4.3 / §4.4 に
> 取り込み済み。** ただし冒頭の「まず定期実行にしない（段階導入）」は
> **発注者判断で撤回**（最初から無人で行く）。判断の根拠は同 §2（`last_to` 単調前進）。

専用PCの実測（`demecal_powershell_probe_guide.md`「ここから決まる制約」）で 2 件出た。
**どちらも Wellfort 側の作業を増やさずに潰す。**

**前提: 2 つとも「無人で定期実行する」場合だけの問題。**
最初は**定期実行にしない**のが一番安全で簡単（`血液CSV取得.bat` をダブルクリック →
取得 → サーバ送信 → ローカル削除 まで一気に終わる）。接続チェックで
**Wellfort 側はすでに同じ操作に 1 回成功している**。自動化はこれが安定してから足す。

#### ① 証明書が `Cert:\CurrentUser\My` にしかない

**証明書は動かさない。動かす必要がない形でタスクを組む。**

| 設定 | 値 | 理由 |
|---|---|---|
| 実行ユーザー | **`info`** | 証明書があるのはこのユーザーのストアだけ |
| 実行条件 | **「ユーザーがログオンしているときのみ実行」** | 対話セッションなので**証明書が確実に見える**。パスワード保存も不要 |
| トリガー | **「ログオン時」＋「毎日 HH:MM」の 2 つ** | どちらか片方だと取りこぼす |
| オプション | **「開始時刻を過ぎた場合すぐに開始」を ON** | PC が落ちていた日も、次のログオン時に自動で走る |

→ **自動ログオン（パスワード自動入力）は不要**。セキュリティを落とさずに済む。

**Wellfort の作業は「セットアップ.bat をダブルクリック 1 回」だけにする。**
タスクスケジューラの画面は開かせない（設定は XML で流し込む）。
**登録直後にその場で 1 回実行して ○/× を表示**すること — ここで黙って失敗すると
数週間気づけない（「サイレント脱落ゼロ」）。

> **やらない: 証明書を LocalMachine へコピー。** 管理者権限が要り、秘密鍵入りの `.pfx` という
> 秘密ファイルを扱うことになり、**そもそも秘密鍵がエクスポート可能かは未確認**
> （プローブは `HasPrivateKey` しか見ていない）。①は設定で回避できるのでこの道は使わない。
> 24時間無人が必要になった場合のみ自動ログオンを検討するが、**セキュリティ判断なので発注者マター**。

#### ② デスクトップが OneDrive 配下（`C:\Users\info\OneDrive\デスクトップ`）

**これは完全にこちら側だけで潰せる。Wellfort の作業はゼロ。**

放置すると**個人情報を含む原本CSVが Microsoft のクラウドへ同期される**。
取込後にローカルを削除しても **OneDrive のごみ箱・バージョン履歴に残り得る**ので、
「原本CSVはPCから削除する」という既存の運用ルール（`demecal_attended_manual_guide.md:114,127`）が
実質的に守れない。**利便性の話ではなく PII の話。**

| | |
|---|---|
| 保存先 | **`C:\demecal\`** に固定（同期対象外・非エンジニアでも見つけやすい） |
| 作れなかったら | `%LOCALAPPDATA%\demecal` へ退避（**OneDrive の既知フォルダ移動の対象外**＝絶対に同期されない） |
| 安全弁 | 解決したパスに **`OneDrive` が含まれていたら書かずに中止**してエラー表示 |
| 後始末 | サーバ送信に**成功したら原本CSVを削除** |
| 表示 | 最後に**実際に使ったパスを必ず出す** |

**接続チェックがデスクトップに出すのは意図どおり**（メールで送ってもらうため／個人情報を
含まないため）。変えるのは**本番のCSV取得だけ**。

**OneDrive 側の設定は変えない（2026-08-31 確定）。** バックアップを停止すると公式の挙動として
**既存ファイルは OneDrive に残り、デバイスのフォルダーからは見えなくなる**
（[Microsoft サポート](https://support.microsoft.com/ja-jp/office/onedrive-%E3%81%A7%E3%83%95%E3%82%A9%E3%83%AB%E3%83%80%E3%83%BC%E3%82%92%E3%83%90%E3%83%83%E3%82%AF%E3%82%A2%E3%83%83%E3%83%97%E3%81%99%E3%82%8B-d61a7930-a6fb-4b95-b28a-6552e77c3057)）。
非エンジニアには**「デスクトップのファイルが全部消えた」**にしか見えず、
影響はその PC の業務全体に及ぶ。会社ポリシーで強制されていれば操作もできない。
**書き先を変えれば済む話なので、相手の PC の設定には触らない。**

**手動運用（attended）側にも同じ危険がある。** 担当者が CSV を**デスクトップに保存**すると
その瞬間に PII がクラウドへ上がる。→ `demecal_attended_manual_guide.md` ステップ②-6 に
**保存先を `C:\demecal\` に固定する**（ブラウザの設定で明示変更）を明記済み。

> **【未確認・重要】現在の手動運用で CSV が実際どこに落ちているかは確認していない。**
> 手順書は「PCに保存される」としか書いておらず**フォルダを指定していない**＝ブラウザの既定任せ。
> OneDrive のバックアップ対象に**ダウンロードは含まれない**（同出典）ので既定のままなら同期されないが、
> それは**一般論**であって、**この PC の保存先がデスクトップ等に変更されていないことの確認にはならない**。
> 「たぶん既定だから安全」で済ませない（一度そう書いて撤回した・2026-08-31）。
> 確認は **エクスプローラーで `Q05-0010` を検索し、場所に `OneDrive` が含まれるか**を見るのが速い。
> 取り込み後に削除する運用なので、**削除済みでも OneDrive のごみ箱・版履歴に残っている**可能性がある。

#### Wellfort にお願いすること

**ダブルクリック 1 回。**（初回のみ、出力された結果ファイルを送ってもらう）
デメカルの **ID / パスワードの受け渡し方法**だけ別途決める（**bat に直書きしない**・§5）。

---

## 5. セキュリティ / 監視

- 秘密: デメカル証明書(OSストア)・ID/PW・`LAB_INTAKE_API_KEY` は機微。**リポジトリ非保存**、OS資格情報/env で保持。
- ローテーション: `LAB_INTAKE_API_KEY` は定期更新（漏洩時は即無効化＝wellfort env 差替のみ）。証明書はCA/クライアントの有効期限を監視。
- 監視: 取得件数・`last_to` 進捗・成否をログ。失敗アラート。PII(健康情報)は要件群のPII方針に準拠。

---

## 6. 未確定（要先方確認）
- 「日付範囲」の基準（報告日/採取日）と反映遅延（§4.2・overview §9.1）。
- レート制限・アクセス時間帯・IP 制限（overview §6）。
- ~~証明書を自動化端末(LocalMachine)へ移設して良いか（完全無人化する場合）~~
  → **不要になった（2026-08-31・§4.4①）**。「ログオン中のみ実行」＋「開始時刻を過ぎたらすぐ開始」で
  回避でき、移設も自動ログオンも要らない。24時間無人が必要になったときだけ再燃する。
- **ログイン後の CSV 一覧 URL とダウンロードリンクの形**（プローブはログインしない設計のため未取得）。
  実装に要るのはこれだけ。**専用PCでの実行が要る**（証明書がその PC にしかない）。
- デメカルの **ID / パスワードの受け渡し方法**（bat に直書きしない）。
