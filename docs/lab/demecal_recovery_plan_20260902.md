...
消えたかどうかは **`/Query /XML` に `<Task` が返らないこと**で見る
(`/Delete` の出力の文言では判定しない — 表示は環境で変わり得るため)。

検査は `C34`〜`C39` が 6 通りの失敗経路それぞれで **失敗 / 削除 1 回 / タスクが残らない /
外部操作 0 回** を固定し、`C11b`〜`C11c` が **正常登録では削除 0 回・タスクは残る**を固定する。
`C40`〜`C42` が「消せなかったとき」の挙動、`C43` が **呼ぶのは `/Create` `/Query` `/Delete` だけ**。
...
#### 検証 `npm run verify:demecal-scheduler`（127 件）
