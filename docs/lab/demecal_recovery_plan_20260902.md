...
### 最終セットアップ BAT（2026-09-03・実装済み・今回の案件専用）

最終実機 1 回のために、**ダブルクリック 1 回**で
「取得範囲の初期化 → 3 本の配置 → タスク登録 (無効)」まで通す 1 ファイル。

```text
?script=final-setup → デメカル自動取得_最終セットアップ_v1.0.bat
                      (ASCII 名 demecal-final-setup-v1.0.bat)
```
...
| | 値 | 置き場所 |
|---|---|---|
| `INITIAL_LAST_TO` | `2026-09-01` | **コード側に固定** (`demecal-final-setup.ts`) |
| `DAILY_AT` | `11:00` | **同上**。だから **`DEMECAL_DAILY_AT` env が未設定でも配れる** |
| `LAB_INTAKE_API_KEY` | Vercel env | 配布生成時に注入。**リポジトリに実値を置かない** |

`ADMIN_API_KEY` は引数にも無いので**通り道が存在しない**。
生成物は取り込み専用キーを平文で含む**機密ファイル**で、配布口は
`cache-control: no-store` / `x-robots-tag: noindex` を返す (既存 `download()` が共通で付ける)。

#### 安全ロジックを書き写さない — 4 段を別プロセスで走らせる

| 段 | 中身 | 出どころ |
|---|---|---|
| [1] | 取得範囲の初期化 | **この案件で新しく書いた唯一のコード** |
| [2] | 3 本 + manifest の配置 | `buildInstallerPayload()` の本文を**バイト一致で同梱** (C-4.1) |
| [3] | タスク登録 (**無効**) | `demecal-scheduler.ps1` を**実行時刻の差し込みだけ**して同梱 (C-5) |
| [4] | `FINAL_SETUP_OK` の表示 | 新規 (表示のみ) |

**[2][3] の中身はこの新ファイルに 1 行も無い。** 検査 A12/A13/A14 が
「同梱した本文が payload と完全一致」をバイトで固定し、
**一致しなければ生成そのものが落ちる** (builder の自己検証)。

**4 段は別々の `powershell.exe` で走らせる。** [2][3] はそれぞれ完結した .ps1 で、
自分で `exit` し `Say` / `Fail` / `Stop-Setup` という**同名の関数**を持つので、
1 セッションへ連結すると **[2] の `exit 0` でそこで全部終わる**し名前も衝突する。
→ `wrapPs1AsBat` が既にやっている「bat が自分自身 (`%~f0`) を読んで
`Invoke-Expression`」を**行範囲ごとに 4 回**行う。各段は自分のプロセスなので
`exit` はその段だけで終わり、**ディスクには何も書かない** (一時 .ps1 を作らない =
鍵を余計な場所へ置かない)。段が落ちたら cmd 側で止めて次へ進めない。

#### [1] の判定ラダー（`force` を絶対に送らない）

```text
last_to が null または < 2026-09-01 → POST → **GET で読み直して確認**
last_to = 2026-09-01               → POST しない・そのまま次へ
last_to > 2026-09-01               → **変更せず STOP** (LAST_TO_AHEAD / exit 1)
```

- **巻き戻さない。** state API は「過去日付なら据置」を返すだけで**エラーにしない**
  (`demecal-state.ts:74`) ので、**API 任せにせず自分で判定して止める**。
- **POST の応答を信じない。** 必ず GET で読み直し、値が一致しなければ
  `STATE_VERIFY_MISMATCH` で止める。
- `$ApiBase` は **`demecal-verify.ps1:58` から読む** (URL をもう 1 か所ベタ書きすると
  移行時に片方だけ直して黙ってずれる)。

#### 検証 `npm run verify:demecal-final-setup`（108 件）

| 層 | 見るもの |
|---|---|
| **A**（34 件） | **書き写していないこと** — 配置/登録の実装語が新ファイルに無い・既存 builder を呼んでいる・同梱本文が payload とバイト一致 |
| **B**（34 件） | 生成物の構造 — cmd 部 ASCII / 4 段の行範囲が隙間なく本文を覆う / 段ごとに別プロセス / 失敗した段で止まる / 鍵が [3][4] と cmd 部に無い |
| **C**（27 件） | **[1] を実際に PowerShell で走らせる** — 判定ラダー 10 通り。通信は stub。**`force` を 1 度も送らないこと**・**叩く先が demecal-state だけ**もここで見る |
| **D**（13 件） | 配布口 — 200 / ファイル名 / `no-store` / **`DEMECAL_DAILY_AT` env が無くても 11:00 が焼き込まれる** / 鍵が無ければ 500 |

#### 退行を注入して落ちることを確認（5 通り）

| 入れた退行 | 結果 |
|---|---|
| A 巻き戻しガードを外す | C13/C14/C15 — **実際に POST が飛んだ**ことを stub が記録 |
| B 書き込み後の読み直しを省く | C3/C6/C21/C22/C23 |
| C `force=true` を送る | **生成時に落ちる** (`禁止された操作が含まれています: force`) |
| D 段の順序を入れ替える | **生成時に落ちる** (`[2] の本文がインストーラ payload と一致しません`) |
| E 失敗しても次の段へ進む | B10 |

C と D が**配布物を作る前**で落ちるのが要点 — 壊れた状態の BAT は生成できない。

#### 実装中に踏んだもの 3 件（すべて実測）

- **禁止語の走査が「やらないと書いた文」に誤爆した (2 回)**。①`demecal-scheduler.ps1:13` の
  「`schtasks /Run` **しない**」 ②こちらが [1] へ生成する「**`force` を送らない**」。
  C-5 (`verify:ps1-order`) / C-6 (`Date.now()` / `STALE_DAYS`) と同じ穴で、これで **3 度目**。
  → 走査は**この builder が書いた実行行だけ**に絞る (`onlyCode`)。
  `force` の保証は「生成物の実行行」と「実際の POST body」の 2 つが持つ。
- **二重引用符の here-string は `$` を展開する**。検査ドライバが payload を `@" … "@` で
  囲っていたため `$false` / `$IntakeKey` が**先に空へ展開され**、
  `Ok = ` という壊れた式になって `Missing statement after '=' in hash literal` で
  payload 全体が parse できなかった。→ `@' … '@` (非展開) へ。
- **禁止語の一覧を持つと禁止検査が自分に落ちる**。新ファイルは
  `for (const banned of [...])` を持つので、その配列を外してから探す。

#### Wellfort に渡す手順（**ダブルクリック 1 回**）

```text
1. デメカル自動取得_最終セットアップ_v1.0.bat をダブルクリック
2. 最後に FINAL_SETUP_OK と出れば完了。画面をそのまま撮って送る
   → 途中で止まった場合は FINAL_SETUP_STOPPED とコードが出るので、
     **やり直さずに**その画面を送る (再実行禁止・修正禁止・追加診断禁止)
3. その後、production runner の手動 1 回だけを別途依頼する
```

**タスクは無効のまま登録される**ので、この BAT だけでは自動取得は始まらない。

#### まだやっていないこと

- **Wellfort 実機での実行**。ローカルの fixture/検査までで止めてある。
- **GitHub Actions の監視ワークフロー** (C-6 の判断どおり、タスク有効化と同時に入れる)。
- **タスクの有効化**。この BAT は `Enabled=false` で登録するだけ。
