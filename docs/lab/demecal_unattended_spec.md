...
| | bat | 目的 | Wellfort の操作 |
|---|---|---|---|
| **①** | `デメカル初回セットアップ_v1.1.bat`<br>（**ファイル名に版が入る**＝担当者が最新版かを目で確認できる。実行時も画面と report.txt に「版 : recon-1.1」が出る） | **本番 bat を書くのに要る情報を 1 回で全部取る**＋②が使う資格情報の保存 | ダブルクリック → **ID/PW を 1 回入力** → 結果を返送 |
