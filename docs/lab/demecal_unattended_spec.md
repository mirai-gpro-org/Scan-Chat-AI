...
**これを本人へ写像する実装が無い**（受け皿カラム `lab_tests.external_test_id` /
`external_barcode` は実在。CLAUDE.md「ID体系の正本」）。
...
| 7 | **`指図番号` から本人（`diagnostic_user_id`）を特定する経路**。EC の注文／キット出荷のどこに `指図番号` が記録されるか。記録されないなら 性別＋生年月日＋採血日 で照合するのか | Wellfort／デメカル（§9 の 9 の前提） |

**1 が最優先。** これが無いと §4.1 の 4 が書けない。
**7 は 1 と並行して確認する。** これが決まらないと §9 の 9（本人写像）に着手できず、
無人化の前提（§2 の ⚠）が埋まらない。
...
| 9 | **血液CSV の行 → `diagnostic_user_id` の写像**（`指図番号` ＋ 性別/生年月日/採血日 から特定）。いまは `test-<時刻>-<連番>` の仮 ID で Elith へ出ている | **未** |
