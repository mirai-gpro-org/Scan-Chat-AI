...
> **【recon-1.6 で確定 2026-09-01】① は担当者に何も入力させない。**
>
...
> — そのとおりで、v1.5 で `Get-Credential`→`Read-Host` に変えたのは
> **入力方式を変えただけ**であり、入力させること自体が誤りだった。
> 接続チェック(probe)が毎回届いていたのは**入力を一切求めないから**である。
>
> → **ID/PW は Vercel env から実行時に取得する**（`GET /api/ops/demecal-cred?k=<PROBE_UPLOAD_TOKEN>`）。
> 配布 bat の対話プロンプトは **0 件**になり、probe と同じ形になった。
>
> | | |
> |---|---|
> | env | `DEMECAL_USER_ID` / `DEMECAL_PASSWORD`（Vercel・Scan-Chat-AI） |
> | 認可 | 既に bat に埋まっている `PROBE_UPLOAD_TOKEN`（`ADMIN_API_KEY` は使わない） |
> | 未設定時 | **503 `not_configured`**。空で返さない（空だとログイン失敗の原因が分かりにくい） |
> | **bat に焼き込まない理由** | 焼き込むとデメカルのパスワードが**平文の .bat のままダウンロードフォルダに残り続ける**。Pマーク対応PC で避けたい／秘匿値は Vercel env に集中管理する確定事項にも反する |
> | PC 上に残るもの | **DPAPI 暗号化された `C:\demecal\secrets\demecal.cred.xml` だけ**（② が再利用するので必要） |
>
> **後始末**: 用が済んだら `PROBE_UPLOAD_TOKEN` と併せて
> `DEMECAL_USER_ID` / `DEMECAL_PASSWORD` も Vercel から消す（3 つ同時に閉まる）。
>
