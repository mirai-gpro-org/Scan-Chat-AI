...
| 配布物 | `デメカル接続チェック_v1.0.bat`（PowerShell を内蔵した 1 ファイル。元は `scripts/demecal-probe.ps1`）。**ファイル名の `_v` は `.ps1` の `$Version` から自動で付く**（担当者が最新版かを目で確認できるようにするため・2026-09-01）。**URL でダウンロードしていただく**（.bat はメール添付も ChatWork も弾かれるため・2026-08-28 実測） |
...
別途お送りする **URL** をブラウザで開くと、`デメカル接続チェック_v1.0.bat` のダウンロードが始まります。
**ファイル名の末尾の版（`_v1.0`）が、お伝えした版と一致しているかご確認ください。**
**デスクトップ**に保存してください。
...
>    `scripts/デメカル接続チェック_v1.0.bat` はプレースホルダ `__PROBE_TOKEN__` のまま。
