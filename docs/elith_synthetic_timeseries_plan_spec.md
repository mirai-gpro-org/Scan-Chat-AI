...
| 根拠(確定仕様) | `docs/elith_s3_data_handoff_spec.md`（§3 フォルダ/命名、§4 format_id、§8 一括書出し/manifest）。`CLAUDE.md`（パス/命名/format_id）。 |
| 状態 | **仕様（本書）。実装は本書確定後。** |
...
> 本書は確定仕様。変更時は本書と `elith_s3_data_handoff_spec.md` を更新してから実装する（CLAUDE.md 作業ルール）。
