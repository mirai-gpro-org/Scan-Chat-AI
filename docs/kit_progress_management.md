...
| 関連文書 | `wellfort_app_design_concept.md` §5 / `test_data_storage_and_db_design.md` §7 / `lab_integration_workflow.md` / `ai_app_ui_ux_proposal.pdf §2.2` (アクション・タスク化) |
...
### 範囲外
- 検査結果データの表示 (→ `test_data_storage_and_db_design.md`)
- ユーザー割当ワークフロー (→ `lab_integration_workflow.md`)
...
### 設計方針
1. **デイリークエスト型** (`ai_app_ui_ux_proposal.pdf §2.2`) のステップ可視化で「次にやること」を明示
...
`ai_app_ui_ux_proposal.pdf §2.2 アクション・タスク化` + プログレッシブ・ディスクロージャーの応用。
...
```
lab_integration_workflow.md Workflow に従い、結果取り込み
   ↓
