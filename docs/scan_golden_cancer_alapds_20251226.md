...
- 取得: **Wellfort が検査機関から手動取得 → admin バッチ（`/api/admin/elith-scan`・単画像）** で処理（`elith_batch_centralization_design.md`）。
...
- **患者情報**: 検査ID K1020 / 生年月日 1960.06.14 / 氏名 新藤幹雄 / 性別 男性 / 受付日 2025.12.17 / 報告日 2025.12.26。
  → **PII 分離**（氏名/生年月日を診断系/S3 に載せない・`data_integration_requirements.md §1.3`）。measurement でない → 除外が正。
