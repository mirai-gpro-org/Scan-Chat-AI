...
| カテゴリ | 対応 |
|---|---|
| iOS Safari | 16+ （**iPhone を最優先：想定ユーザーの半数以上**） |
| iPadOS Safari | 16+ |
| Android Chrome | 直近2バージョン |
| Desktop Chrome / Edge / Firefox | 直近2バージョン |
| カメラ | 背面カメラ優先（無ければ前面でフォールバック許容） |
| 画面 | 縦長モバイル前提（提案書ワイヤーフレーム準拠） |

#### 5.4.1 デバイス別の利用シーン

スキャン機能は **スマートフォン（特に iPhone）が最適デバイス**。手持ちで背面カメラを使った紙文書スキャンに最も向くため。タブレット / PC は補助的に対応する。

| デバイス | スキャン利用 | 備考 |
|---|---|---|
| スマホ (iPhone / Android) | ◎ 主用途 | 背面カメラ + 手持ち、AR ハイライト最適化 |
| タブレット | ○ | 据置スタンドでの利用シーン。AR 検知間隔は同条件 |
| PC | △ | Web カメラ品質に依存。UI 上で「スマホでの撮影を推奨」ヒント表示 |

クロスデバイス連携の全体方針（Google One Tap、セッション継続、デバイス引継ぎ）は `docs/device_and_auth_requirements.md` を参照。スキャン結果は Supabase 経由で他デバイスに同期され、診断結果は**タブレット / PC で精読**することを想定する。
