...
#### 5.6.1 発症予測テーブルは「固定疾患ラベル」をテンプレとする（2026-08・元PDF画像で確認）
- **実測(元PDF p.2 を画像化して確認)**: 発症予測テーブルの **値(5年/10年/相対リスク比)は正確・順序も正しい** が、
  **疾患名セル(独自エンコードフォント)をモデルがOCRできず、"カテゴリ的にありがちな別病名"を幻覚**する
  (run毎に別名。例: 痛風→慢性腎臓病 / 労作性狭心症→心房細動 / 急性心筋梗塞→弁膜症 / 脳梗塞→末梢動脈疾患 / 肺がん↔胃がん入替)。
  pdftotext でも病名は文字化け＝同じ原因。**プロンプトで「名前を読め」と言っても字が読めないので直らない。**
- **この様式の疾患名・順序・カテゴリは固定**で、しかも **アドバイスページ(p.3〜9)では同じ18疾患名が正確に読める**(2ソースで確定)。
  → **発症予測テーブルは固定疾患リストをラベルの正とし、値を上から順(カテゴリ別)に対応付ける**(名前=印字どおりの正名・値=画像)。
  固定リスト(18疾患): 生活習慣病=糖尿病/高血圧/脂質異常症/痛風/鉄欠乏性貧血、循環器疾患=労作性狭心症/急性心筋梗塞/心不全/閉塞性動脈硬化症/脳梗塞、
  悪性腫瘍=肺がん/大腸がん/胃がん/すい臓がん/子宮頸がん/乳がん/前立腺がん、神経疾患=うつ病。**値3つとも空欄の行は出力しない**(男性の子宮頸/乳がん等)。
  実装=`AI_PREDICTION_USER`。統合(名前完全一致)でアドバイスと1疾患へ結合される。
- **前提と残リスク**: 固定リストの正しさは「値が正しい順序で読めていること」に依存する。LAiF が疾患・順序を改訂したら固定リストがズレる
  → **改訂検知(件数/カテゴリ構成の乖離チェック)＋固定リスト更新** は今後の課題(§5.6)。値の欠落/順序乱れが起きた run は🔍/🎯で検出する運用。
- **1ページ目(表紙)は読み方の説明＋サンプル『来富太郎様』のデモ抜粋表**(生活習慣病6疾患・別人値: 糖尿病21.5%等)。**本人の実値は2ページ目**(糖尿病4.1%等)。
  → **表紙の抜粋表は発症予測itemに入れない**(`AI_PREDICTION_USER`「表紙では発症予測itemを出さない」)。これが無いと統合が表紙のデモ値を先頭6疾患に採用し
  🎯golden(2ページ目)と食い違う(実測 2026-08: expected16/correct10/wrong6)。表紙除外で先頭6疾患が本人値になり🎯一致する見込み。
