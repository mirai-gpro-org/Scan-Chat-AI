...
| バージョン | 0.1 (Draft) |
| 作成日 | 2026-07-05 |
| 目的 | AIスキャン→Elith 形式 JSON→S3 の処理を、**API キーを Vercel で一元管理**しつつ、ユーザーアプリ・admin バッチの入口で共通利用できるようにする |
| キー方針 | **Gemini / AWS(S3) キーは Vercel の環境変数に集約。ローカル `.env`・operator PC・クライアントには置かない** |
| 関連文書 | `docs/elith/elith_s3_data_handoff_spec.md` / `docs/elith/batch_scan_to_elith_usage.md` / `docs/lab/wellfort_admin_lab_upload_spec.md` / `docs/subscription/subscription_management_feature_requirements.md` (要件2/4) |
...
| (c) CLI (単発) | 運用端末 | (ローカルにキーが無いため**利用不可**) | — |

- **キーは Vercel に集約 (ローカル `.env` には一切置かない)** ことが確定方針。
- **恒常運用は (b) admin バッチに寄せる** → キーは Vercel の 1 箇所だけ。operator はブラウザで操作。

### 3.4 「キーが Vercel のみ」の帰結 (重要)

ローカル `.env` にキーが無いため、**単発 CLI (`scripts/batch-scan-to-elith.mjs`) はローカルでは動作しない**
(Gemini/AWS キーを読めない)。したがって:

- **今回の 61 サンプルも、admin バッチ (サーバ実行) 経由で処理する**のが正道。
  operator はブラウザで NAS 上の画像を選択 → サーバ (Vercel) がスキャン+S3 (Vercel のキーを使用)。
- CLI は「キーが利用できる実行環境」でのみ有効。例: **Vercel Cron / サーバ上のジョブ**として動かすなら
  Vercel の env を参照できるので可。ローカル端末での直実行は不可 (キーを配らない方針のため)。
- → **実装の主対象は (b) サーバ側 admin バッチ**。CLI は将来 Vercel 実行に載せる場合の補助に留める。

### 3.5 Vercel 実行モデルの制約 (per-image 処理)

Vercel の関数には実行時間上限があるため、**1 リクエストで全件を回さない**。既存 `/api/scan` と同様に
**1 画像 = 1 リクエスト**で処理し、ブラウザ側が選択ファイルを順に呼ぶ (進捗表示・失敗リトライ)。
キーはサーバ側にのみ存在し、ブラウザはファイルを送るだけ。

### 3.3 共有モジュールの責務 (案)

`src/lib/elith-export.ts` (サーバ専用):
- 入力: 画像 (Buffer) or 確定 Markdown、`format_id`、`client_id`、`test_date`(任意)。
- 処理: (画像なら) Gemini スキャン → Markdown → Elith エンベロープ (§`docs/elith/elith_s3_data_handoff_spec.md`) 生成。
...
`docs/lab/wellfort_admin_lab_upload_spec.md` の既存アップロード UI を拡張/隣接させる。
...
- 画面: `/admin/elith-batch` (または既存 `/admin/lab-results/upload` に「Elith出力」動作を追加)
  ...
  - **顧客割当**: 実顧客の `diagnostic_user_id` に紐付け (サンプルと違い本番は実ユーザー。
    `docs/lab/lab_integration_workflow.md` の割当ワークフロー準拠。氏名OCRのみの自動確定は禁止)
...
| 2 | 本番 client_id = 実顧客 `diagnostic_user_id` の割当タイミング/UI (`docs/lab/lab_integration_workflow.md`) |
| 3 | がんリスク/遺伝子の元ファイルは画像(AIスキャン対象)か PDF(抽出)か。PDF の構造化方式 |
| 4 | ~~キー保管先~~ → **確定: Vercel の環境変数で一元管理 (ローカル `.env` には置かない)**。CLI はローカル実行不可 → admin バッチ(サーバ)で処理 |
