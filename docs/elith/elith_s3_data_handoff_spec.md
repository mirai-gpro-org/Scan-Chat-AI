...
| 用語 | 定義 | 備考 |
|---|---|---|
| `client_id` | **顧客 (利用者) を一意に識別する ID**。Wellfort の `diagnostic_user_id` (uuid) を充てる | 顧客系/診断系を橋渡しする唯一のキー。**PII を含まない** (`docs/architecture/data_integration_requirements.md §1.3`) |
...
- 顧客系 DB (氏名・住所・生年月日等の PII) と診断系 DB を結ぶ**唯一の橋渡しキー**であり、
  それ自体は PII を含まない (`docs/lab/lab_integration_workflow.md §1.1`)。
...
`docs/architecture/data_integration_requirements.md §1.3` / `docs/lab/lab_integration_workflow.md §1.1` の**PII 越境禁止**に準拠する。
...
- **`BloodTestData`** (デメカル新様式 2026-07〜): 決定論パース。`項目名N` ヘッダの標準名を `name`・データ行の略号を `name_detail`。
  ...
  - **時系列 (§3.3 年複数回)**: 同一 client_id で複数採血日を持つ場合、納品セット アセンブリは**時系列を丸ごと納品**する
    (各採血日を自分の date フォルダへ)。自動選択は**時系列を持つ client を優先**採用する。
...
> **model は `CABA-v5.4`** (旧 v4d から更新)。ラップ詳細は `docs/elith/elith_assembly_wrapping_spec.md` を参照。
...
[検査機関結果] 検査機関 → Wellfort 代理アップロード
   ...
   (docs/lab/lab_integration_workflow.md)              │
                                              ▼
