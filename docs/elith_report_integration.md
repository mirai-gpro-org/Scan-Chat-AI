...
| 関連文書 | `diagnostic_session_data_spec.md §3.4` (JSON フォーマット定義) / `test_data_storage_and_db_design.md §6.4` (3 モード表示) / `wellfort_app_design_concept.md §2` (4 UI アプローチ) / `data_integration_requirements.md §6 EF-6` (callback) |
| 実サンプル | `docs/2026_05_24 Elith_demo.json` |
...
### 範囲外
- JSON 構造そのもの (→ `diagnostic_session_data_spec.md §3.4`)
- Elith 側の推論ロジック (→ Elith 社内資料)
- 検査結果 PDF/scan_md の保管 (→ `test_data_storage_and_db_design.md`)
...
→ `result` 内側が `diagnostic_session_data_spec.md §3.4` 定義の配列。
...
`data_integration_requirements.md §5` の既存テーブルに以下のカラムで保管:
...
`test_data_storage_and_db_design.md §6.4` の 3 モード設計を Elith レポートに適用:
...
- 我々の **a) サマリー LLM 生成コストはゼロ** (`test_data_storage_and_db_design.md §6.4` で見積もった ~$0.008 が不要)
- 表示瞬時 (LLM 待ちなし)
- ハルシネーション源が 1 つ減る (Elith の品質に委譲)

### 4.2 適用判定 (5 pg 以上ルールへの当てはめ)

Elith レポートは合計 ~21,000 字 ≈ **10〜15 ページ相当**。`test_data_storage_and_db_design.md §6.4` の **5 ページ以上で 3 モード自動 ON** ルールに該当 → **常に 3 モード適用**。

---

## 5. ダッシュボードカードへの二次抽出

`wellfort_app_design_concept.md §2` の **4 UI アプローチ**にあるダッシュボード要素を、Elith JSON から二次抽出する:
...
- [ ] a/b/c 3 モード切替 UI (汎用、`test_data_storage_and_db_design.md §6.4` に倣う)
- [ ] ダッシュボードカード描画 (4 UI アプローチ、`wellfort_app_design_concept.md §2`)
- [ ] `【sub】` `[N]` パース
- [ ] Realtime 通知
...
| Ver | 日付 | 内容 |
|---|---|---|
| 0.1 | 2026-05-28 | 初版。実サンプル `docs/2026_05_24 Elith_demo.json` (10 セクション 21K 字) を解析、3 モード設計との対応 (a=アブストラクト/b=医療受診目安+抽出/c=全文)、ダッシュボード 4 UI への二次抽出ロジック、`diagnosis_results` + `diagnosis_result_items` テーブル、`【】` `[N]` パース戦略、Phase 別実装計画を規定 |
