...
| 関連文書 | `wellfort_app_design_concept.md` §5b / `diagnostic_session_data_spec.md` §3.2 / `data_integration_requirements.md` §1.3 |

---

## 1. 背景と課題

`wellfort_app_design_concept.md §5 (b)` の通り、Wellfort 側が代理アップロードする検査結果は**固定フォーマット**で、以下 4 種類:
...
| 制約 | 出典 |
|---|---|
| `diagnostic_user_id` が両系統 (顧客系 / 診断系) の**唯一の橋渡し** | `data_integration_requirements.md §1.3-2` |
| **PII 越境禁止** — 氏名・住所・生年月日は診断系 DB に書き込まない | `data_integration_requirements.md §1.3-7` |
