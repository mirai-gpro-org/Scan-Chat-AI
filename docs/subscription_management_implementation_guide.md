...
| バージョン | 0.1 (Draft) |
| 作成日 | 2026-06-28 |
| 対象 | `subscription_management_feature_requirements.md` (要件1〜4・データモデル拡張) の実装 |
| 関連文書 | (要件) `subscription_management_feature_requirements.md` / (S3) `elith_s3_data_handoff_spec.md` / (UP) `wellfort_admin_lab_upload_spec.md` / (進捗) `kit_progress_management.md` / (割当) `lab_integration_workflow.md` |
...
- [ ] 「検査キット進捗」セクション追加: 回 × 検査種別 × 進捗6段階 (`kit_progress_management.md §4.2`)
...
既存 `/admin/lab-results/upload` (`wellfort_admin_lab_upload_spec.md`) を中核に、契約/回と連動。
...
- [ ] `lab_integration_workflow.md` の Workflow 1/2/3 に従い `diagnostic_user_id` を確定
...
- 新規: `src/lib/interview-outputs.ts`
  ...
  - [ ] **Elith向け JSON**: `LifestyleQuestionnaireData` (`elith_s3_data_handoff_spec.md §7.3`)
...
データ仕様は `elith_s3_data_handoff_spec.md` 確定済。**出揃い判定 → 一括書き出し**を実装。
...
- 新規: `src/lib/elith-handoff.ts`
  - [ ] プランが要求する format_id 集合 (`plan_test_items`) を充足したか判定 (`elith_s3_data_handoff_spec.md §8.1`)
...
- [ ] **エンドツーエンド**: 契約 → 発送 → 受取/返送 → 結果UP → 問診 → 出揃い判定 → S3一括 → (Elith診断結果受信は `elith_report_integration.md`)
