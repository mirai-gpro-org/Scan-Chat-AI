...
- 変更履歴:
  ...
  - **v0.3: HP/EC 実スキーマ（`database-schema.md`）に整合。`lab_delivery` 廃止、`kit_shipment` を `orders` 由来に、本人解決を email 突合に。`docs/hp_ec/HP回答への返答_v0.3.md` と対。**
- 関連: `docs/architecture/data_integration_requirements.md`（PII分離原則）, `docs/architecture/test_data_storage_and_db_design.md`, `docs/hp_ec/HP回答への返答_v0.3.md`
