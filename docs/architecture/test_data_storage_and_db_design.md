...
| 関連文書 | `docs/architecture/wellfort_app_design_concept.md` §5 / `docs/lab/lab_integration_workflow.md` / `docs/architecture/diagnostic_session_data_spec.md` §3 / `docs/architecture/data_integration_requirements.md` |
...
| **範囲外** | UI / ダッシュボード設計 (→ `docs/architecture/wellfort_app_design_concept.md`)、ユーザー割当ワークフロー (→ `docs/lab/lab_integration_workflow.md`)、scan_md フォーマット詳細 (→ `docs/architecture/diagnostic_session_data_spec.md` §3.2) |

---

## 2. データソースの分類

`docs/architecture/wellfort_app_design_concept.md §5` の 3 ソースを再掲しつつ、本書での取扱いを明示:
...
| フォーマット | 構造化 JSON + Markdown (`docs/architecture/diagnostic_session_data_spec.md §3.1`) |
...
詳細フロー: `docs/lab/lab_integration_workflow.md` Workflow 1/2/3
...
> **確定 (2026-08-20・発注者承認): 案C′ — 原本は S3 `ap-northeast-1`。**
> 本節は当初「S3 / Supabase Storage」の両論併記だったが、次のとおり確定した。
>
> | 層 | 置き場所 | 理由 |
> |---|---|---|
> | DB・ホット層 | **Supabase (現行のまま)** | Vercel iad1 と近接。移すと SSR の逐次クエリが遅くなる |
> | 原本ファイル | **S3 `ap-northeast-1`** | §6.1 の「10 年保管・削除不可」を Object Lock で技術担保。データ主権要件 (`要件定義書.md:208`) にも寄せられる |
>
> 署名 URL でブラウザとストレージが直結するため、**Vercel のリージョンは配信経路に入らない**
> = アプリのレイテンシを悪化させずに原本だけ国内化できる。
> コスト比較 (AWS Price List API / Supabase 公開料金・2026-08-20 取得) では
> 月あたり数ドル〜20 ドルの差にしかならず、**判断軸はコストではなく削除不可の担保**。
>
> - 実装: `src/lib/originals-storage.ts` (S3 未設定なら Supabase へフォールバック)
> - 構築手順: `docs/operations/S3原本ストレージ_構築手順書.md`
> - 読み出しは `storage_url` が `s3://` かどうかで自動的に振り分ける (段階移行できる)


`docs/architecture/system_architecture_overview.md §1.2` の PII 物理分離原則を踏襲。
...
- 顧客系 → 診断系の橋渡し時点で `docs/lab/lab_integration_workflow.md` の Workflow 1/2/3 が走る

### 6.4 結果表示モード設計 (3 モード: サマリー / 要注意抜粋 / 全編)

#### 背景

207 ページの遺伝子検査 PDF を生のまま表示すると、`docs/proposals/ai_app_ui_ux_proposal.pdf` 末尾の「スクロール地獄」アンチパターンに直撃する。一方、ユーザーは「LLM サマリーだけ」では信頼しきれない。**両者を両立**するため、長尺レポート用の **3 モード表示**を採用。
...
`docs/architecture/data_integration_requirements.md §2` の「2 Supabase 分離」を継承して新テーブルを配置:
...
create table lab_companies (
  ...
  contact_email      text,
  workflow_default   int not null default 2,           -- 1 | 2 | 3 (docs/lab/lab_integration_workflow.md)
...
> 詳細は `docs/lab/kit_progress_management.md §8` を参照 (本書では関心の分離のため進捗管理仕様は分離)。
...
5. ユーザー紐付け (docs/lab/lab_integration_workflow.md の Workflow を実行)
   Workflow 2 例:
...
9. Elith 通知 (バッチ)
   ...
   - Elith AI バッチに渡す (docs/architecture/data_integration_requirements.md EF-5)
```
...
3. ユーザーがセル単位で検証 (docs/scan/scan_feature_requirements.md §5)
   (実装済)
...
→ Phase 1 移行時に各検査会社と**配信プロトコル契約**を結ぶ必要あり (`docs/lab/lab_integration_workflow.md §5` 実装ロードマップ参照)
